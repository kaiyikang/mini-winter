...
在掌握 Java 基础语法后，我发现主流框架（如 Spring Boot）高度抽象的「约定优于配置」机制虽然提升了开发效率，却也像一个「黑盒」，屏蔽了底层复杂的实现细节。为了将这些黑盒透明化，我决定学习如何从底层构建一个简化版框架。

这一过程不仅让我深入理解了属性解析、注解驱动的 Bean 扫描及生命周期管理，更让我掌握了 AOP 的底层代理机制与 Web 容器的协作原理。通过手写代码，我直面并解决了循环依赖、声明式事务的原子性保证以及类加载器隔离等实际生产环境中的核心挑战。
